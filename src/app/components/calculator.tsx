...
export default function Calculator() {
    ...
    const [inputs, setInputs] = useState({
      ...
      const handleSubmit = () => {
        console.log(inputs)
        let calculate = HandleCalculate(inputs.length, inputs.width, inputs.height, inputs.units)
  ...
  return (
    <div className="bg-white  w-[500px] border-1 rounded-lg bg-white p-10">
      ...
      <div className="mt-5">
        ...
        >
            <option disabled value="" defaultValue={'selecione'}>Selecione...</option>
 ...
 Calcular
        ...
        <span className={`mr-8 ${result !== undefined ? 'block' : 'hidden'}` }>Peso volumetrico: {result} </span>
        </div>
    </div>
  );
}
